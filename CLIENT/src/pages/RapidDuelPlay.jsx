...
export default function RapidDuelPlay({ user }) {
  ...
  // state from RapidDuel matchmaking navigate()
  ...
  useEffect(() => {
    ...
    // time over => submit timeout answer (-1)
    setLocked(true);
  ...
  const goNextAfterDelay = () => {
    setTimeout(() => {
      if (idx >= total - 1) {
        setWaiting(true); // waiting for opponent result
        return;
      }
      setIdx((i) => i + 1);
    }, 800);
